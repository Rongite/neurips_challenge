...
from .constants import LABEL_LOOKUP, REVERSED_LABEL_LOOKUP, BI_REVERSED_LABEL_LOOKUP
...
class ElphHashes(object):
    ...
    def _get_bi_intersections(self, edge_list, hash_table):
        """
        extract set intersections as jaccard * union
        @param edge_list: [n_edges, 2] tensor to get intersections for
        @param hash_table:
    ...
    def get_bi_subgraph_features(self, links, hash_table, cards):
        """
        extracts the features that play a similar role to the labeling trick features. These can be thought of as approximations
        of path distances from the source and destination nodes. There are k+2+\sum_1^k 2k features
        @param links: tensor [n_edges, 2]
        ...
        if links.dim() == 1:
            links = links.unsqueeze(0)
        intersections = self._get_bi_intersections(links, hash_table)
        ...
        if self.max_hops == 1:
            ...
            for i in range(1, self.max_hops + 1):
                for j in range(1, self.max_hops + 1):
                    f_index = BI_REVERSED_LABEL_LOOKUP[self.max_hops][(i,j)]
                    features[:, f_index] = intersections[(i,j)]
                    for i1 in range(1, i + 1):
                        for j1 in range(1, j + 1):
                            if i != i1 or j != j1:
                                features[:, f_index] = features[:, f_index] - features[:, BI_REVERSED_LABEL_LOOKUP[self.max_hops][(i1,j1)]]

            # compute B count features
            for i in range(1, self.max_hops + 1):
                f_index = BI_REVERSED_LABEL_LOOKUP[self.max_hops][(i, 0)]
                features[:, f_index] = cards1[:, i - 1]
                if (i > 1):
                    features[:, f_index] = features[:, f_index] - features[:, BI_REVERSED_LABEL_LOOKUP[self.max_hops][(i  - 1, 0)]]
                for j in range(1, self.max_hops + 1):
                    a_index = BI_REVERSED_LABEL_LOOKUP[self.max_hops][(i, j)]
                    features[:, f_index] = features[:, f_index] - features[:, a_index]

            for i in range(1, self.max_hops + 1):
                f_index = BI_REVERSED_LABEL_LOOKUP[self.max_hops][(0, i)]
                features[:, f_index] = cards2[:, i - 1]
                if (i > 1):
                    features[:, f_index] = features[:, f_index] - features[:, BI_REVERSED_LABEL_LOOKUP[self.max_hops][(0, i - 1)]]
                for j in range(1, self.max_hops + 1):
                    a_index = BI_REVERSED_LABEL_LOOKUP[self.max_hops][(j, i)]
                    features[:, f_index] = features[:, f_index] - features[:, a_index]
