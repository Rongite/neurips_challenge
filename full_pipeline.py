...
def save_stage1_evaluation_report(stage1_results: Dict[str, Dict], data_ranges: Dict[str, Dict[str, float]], save_dir: Path):
    ...
    report_path = save_dir / "stage1_evaluation_report.csv"
...
def main():
    ...
    print("\n" + "="*60)
    print("STAGE 1: Training with 8:1:1 data split")
    print("="*60)
    ...
    print(f"\n" + "="*60)
    print(f"STAGE 1 EVALUATION SUMMARY:")
    ...
    print(f"\n" + "="*60)
    print("STAGE 2: Generating final predictions")
    print("="*60)
    ...
    for idx, tgt in enumerate(TARGETS):
        ...
        preds=[]
        with torch.no_grad():
            for b in tqdm(dl, desc=f"Predicting {tgt} on test set (Stage 2 model)", leave=False):
    ...
    for target, result in stage1_results.items():
        eval_info = result['evaluation']
        print(f"   {target}: {eval_info['grade']} ({eval_info['level']}) - {eval_info['relative_error']} error")
    print(f"Stage 2 models used for final submission")
    ...
    print("Saved submission.csv (using Stage 2 model predictions)")
